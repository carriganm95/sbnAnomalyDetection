"""Train the PMT autoencoder (Option A – standalone job).

Usage::

    python -m sbn_anomaly_detection.train.train_pmt --config configs/pmt_train.yaml
...
from sbn_anomaly_detection.models.model import PMTAutoencoder
...
def train(cfg: dict) -> None:
    ...
    logger.info("Training PMT autoencoder on device: %s", device)
    ...
    norm_path = Path(cfg["training"]["checkpoint_dir"]) / "pmt_normalizer.npz"
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(model, optimizer, epoch, val_loss, ckpt_dir, "pmt_best.pt")

    save_checkpoint(model, optimizer, epochs - 1, val_loss, ckpt_dir, "pmt_last.pt")
    logger.info("PMT training complete. Best val loss: %.6f", best_val_loss)


def main() -> None:
    parser = argparse.ArgumentParser(description="Train PMT autoencoder")
